...
from quantize_activations.test_grad import grad_diff_roberta
...
class MyTrainer(Trainer):
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]], step) -> torch.Tensor:
        """
        ...
        if step <= 1:
            # print(f'Gradient SQNR {grad_diff_roberta(model, self.compute_loss, inputs)}.')
            ...
            buffer_size = torch.cuda.memory_allocated() - before
            # print(prof.key_averages().table(sort_by='cuda_memory_usage', row_limit=200))
