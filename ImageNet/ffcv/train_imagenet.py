...
Section('training', 'training hyper param stuff').params(
    ...
    quan=Param(int, 'use quantization?', default=0)
)
...
class ImageNetTrainer:
    ...
    @param('data.ffcv')
    def __init__(self, gpu, distributed, ffcv):
        ...
        print(f'Using ffcv: {ffcv}')
        self.train_loader = self.create_train_loader() if ffcv else self.create_train_loader_traditional()
    ...
    def create_train_loader(self, train_dataset, num_workers, batch_size,
        ...
        print(f'Using {this_device}!! {ch.cuda.is_available()}')
    ...
    @param('training.distributed')
    @param('data.in_memory')
    def create_train_loader_traditional(self, train_dataset, num_workers, batch_size,
                            distributed, in_memory):
        this_device = f'cuda:{self.gpu}'
        print(f'Using {this_device}!! {ch.cuda.is_available()}')
        ...
        traindir = "/datasets01_ontap/imagenet_full_size/061417/train"
        import torchvision
        ...
        data_loader = ch.utils.data.DataLoader(train_dataset,batch_size=batch_size,sampler=train_sampler,
        num_workers=num_workers,pin_memory=in_memory)
        return data_loader
    ...
    @param('training.quan')
    def create_model_and_scaler(self, arch, pretrained, distributed, use_blurpool, quan):
        scaler = GradScaler()
        if quan:
    ...
    def train_loop(self, epoch, log_level):
        ...
        for ix, (images, target) in enumerate(iterator):
            ### Training start
            if self.ffcv:
                images, target = images.to(f'cuda:{self.gpu}', non_blocking=True), target.to(f'cuda:{self.gpu}', non_blocking=True)
    ...
    def initialize_logger(self, folder, arch):
        ...
        if self.gpu == 0:
            folder = (Path(folder) / arch/ str(self.uid)).absolute()
